...
%% Folder containing TIFF image stacks
% All .tif and .tiff files in this folder will be processed
img_folder = '/home/joshscurll/Documents/MATLAB/Research/Madison/';
...
save_tiffs = true; % true or 1 to save new TIFF stacks, false or 0 to not save.

% Specify whether to save all results to a MAT-file
save_all = true; % true or 1, or false or 0.

% Specify whether to save total_intensity to a csv file
save_csv = true; % true or 1, or false or 0.
...
%% Show figures?
show_figs = false; % 1 or true, or 0 or false
...
file_struct = dir(fullfile(img_folder, '*.tif*'));
numfiles = size(file_struct,1);

for fnum = 1:numfiles
    
    img_fullpath = fullfile(img_folder,file_struct(fnum).name);
    ...
    if save_all
        save(fullfile(save_folder,[fname,'_processed.mat']), 'sigmas', ...
            'scales', 'img_fullpath', 'img', 'IMG', 'mask', 'total_intensity', ...
            'threshold_intensity', 'threshold_LoG', 'threshold_LoG_diff', ...
